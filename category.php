<?
include('session.php');
if ($_GET['category'] != 'Dog' && $_GET['category'] != 'Cat' && $_GET['category'] != 'Bird' && $_GET['category'] != 'Fish' && $_GET['category'] != 'Hamster' && $_GET['category'] != 'Turtle' && $_GET['category'] != 'Other') {
...
<div class="container">
		...
		$limit = 12;
		$offset = $limit * ($page-1);
		$result = mysqli_query($connection, "SELECT * FROM pets WHERE petcategory like '%".mysqli_real_escape_string($connection, $_GET['category'])."%' AND adopted = 0 ORDER BY petid DESC LIMIT $offset, $limit");
		$i = 0;
		$r = 0;
